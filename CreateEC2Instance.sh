...
AvaiZone=$(aws ec2 describe-subnets --subnet-ids $Subnet1 | grep AvailabilityZone | awk '{print $2}'| sed 's/"//g' | sed 's/,//g')
echo $AvaiZone

Subnet2=$(aws ec2 create-subnet --vpc-id $VpcId --cidr-block 10.10.2.0/24 | grep SubnetId | awk '{print $2}'| sed 's/"//g' | sed 's/,//g')
echo $Subnet2
...
echo $SecGroup
d=$(aws ec2 authorize-security-group-ingress --group-id $SecGroup --protocol tcp --port 22 --cidr 0.0.0.0/0)
...
###creating a CloudWatch Alarm based on CPU utilization
...
StausId1=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=MyInstance" --query "Reservations[].Instances[].State.Name" | sed 's/"//g' | sed 's/[][]//g')
echo $StausId1

######creating an Autoscaling group for EC2 Instance:

aws autoscaling create-auto-scaling-group --auto-scaling-group-name my-asg --launch-configuration-name my-lc-from-instance --load-balancer-names my-load-balancer --min-size 1 --max-size 2 --vpc-zone-identifier "$Subnet1,$Subnet2"

sleep 5
StausId2=$(aws ec2 describe-instances --filters --query "Reservations[].Instances[].State.Name" | sed 's/"//g' | sed 's/[][]//g')
echo $StausId2
